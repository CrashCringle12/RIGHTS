...
function USWN.Load()
	...
	--UpLeft and UpRight should use UpLeft as Head
	if string.find(sElement, "Head") and 
	( sButton == "UpLeft" or 
	sButton == "UpRight" ) then
		Button = "UpLeft";
	end
	...
	--Setting so that UpLeft and UpRight Heads dont turn
	if sElement == "Explosion" or ( string.find(sElement, "Head") and 
	( sButton == "UpLeft" or 
	sButton == "UpRight" )) then
		t.BaseRotationZ = nil;
		--Flipping the UpRight head horizontal
		if sButton == "UpRight" then
			t.BaseRotationY = 180;
		end
	end
